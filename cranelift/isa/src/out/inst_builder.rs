...
pub trait InstBuilder<'f>: InstBuilderBase<'f> {
    ...
    fn jump(mut self, block_call_label: ir::Block, block_call_args: &[Value]) -> Inst {
        let block_call = self
            .data_flow_graph_mut()
            .block_call(block_call_label, block_call_args);
    ...
    fn brif(
        ...
        let block_then = self
            .data_flow_graph_mut()
            .block_call(block_then_label, block_then_args);
        let block_else = self
            .data_flow_graph_mut()
            .block_call(block_else_label, block_else_args);
        let ctrl_typevar = self.data_flow_graph().value_type(c);
        self.Brif(Opcode::Brif, ctrl_typevar, block_then, block_else, c)
            .0
    }
    ...
    fn resumable_trapnz<T1: Into<ir::TrapCode>>(self, c: ir::Value, code: T1) -> Inst {
        let code = code.into();
        let ctrl_typevar = self.data_flow_graph().value_type(c);
        self.CondTrap(Opcode::ResumableTrapnz, ctrl_typevar, code, c)
            .0
    }
    ...
    fn return_call_indirect(mut self, SIG: ir::SigRef, callee: ir::Value, args: &[Value]) -> Inst {
        ...
        self.CallIndirect(Opcode::ReturnCallIndirect, ctrl_typevar, SIG, vlist)
            .0
    }
    ...
    /// - a: A SIMD vector type
    #[allow(non_snake_case)]
    fn insertlane<T1: Into<ir::immediates::Uimm8>>(
        self,
        x: ir::Value,
        y: ir::Value,
        Idx: T1,
    ...
    fn dynamic_stack_store(self, x: ir::Value, DSS: ir::DynamicStackSlot) -> Inst {
        let ctrl_typevar = self.data_flow_graph().value_type(x);
        self.DynamicStackStore(Opcode::DynamicStackStore, ctrl_typevar, DSS, x)
            .0
    }
    ...
    #[allow(non_snake_case)]
    fn uadd_overflow_trap<T1: Into<ir::TrapCode>>(
        self,
        x: ir::Value,
        y: ir::Value,
        code: T1,
    ...
    #[allow(non_snake_case)]
    fn bitcast<T1: Into<ir::MemFlags>>(
        self,
        MemTo: crate::ir::Type,
        MemFlags: T1,
        x: ir::Value,
    ...
    /// - a: Value atomically loaded
    #[allow(non_snake_case)]
    fn atomic_load<T1: Into<ir::MemFlags>>(
        self,
        AtomicMem: crate::ir::Type,
        MemFlags: T1,
        p: ir::Value,
    ...
    /// - p: An integer address type
    #[allow(non_snake_case)]
    fn atomic_store<T1: Into<ir::MemFlags>>(
        self,
        MemFlags: T1,
        x: ir::Value,
        p: ir::Value,
        ...
        let MemFlags = MemFlags.into();
        let ctrl_typevar = self.data_flow_graph().value_type(x);
        self.StoreNoOffset(Opcode::AtomicStore, ctrl_typevar, MemFlags, x, p)
            .0
    }
    ...
    fn AtomicCas(
        ...
        debug_assert_eq!(
            opcode.format(),
            InstructionFormat::from(&data),
            "Wrong InstructionFormat for Opcode: {}",
            opcode
        );
    ...
    fn UnaryImm(
        ...
        let mut data = ir::InstructionData::UnaryImm { opcode, imm };
        data.sign_extend_immediates(ctrl_typevar);
        debug_assert_eq!(
            opcode.format(),
            InstructionFormat::from(&data),
